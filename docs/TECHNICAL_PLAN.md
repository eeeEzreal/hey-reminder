...
5. 默认阈值为 10 分钟；每次达到当前到期时间只产生一个 `ReminderConditionReachedEvent`，通知操作确认后才安排下一次到期时间。
...
- 通知提供“收到”“再给我 5 分钟”和“暂停 30 分钟”；三个默认时长来自统一的 `ReminderTimingConfig`，Phase 8 可直接接入持久化设置。
